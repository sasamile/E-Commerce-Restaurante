...
function Sidebar({ showMenu, setShowMenu, toggleCarrito }: SidebarProps) {
  ...
  return (
    <>
      <div
        className={`bg-[#1F1D2B] fixed w-[7.5rem] h-full text-white text-4xl flex flex-col justify-between items-center  py-8 z-10 p-6  max-lg:-left-full ${
          showMenu && "max-lg:left-0"
        }`}
      >
        <div>
            ...
            {SidebarRout.map(({ href, Icon, label }, index) => {
              ...
              return (
                <li
                  ...
                  className={`mx-auto ${
                    isActive &&
                    "bg-[#262837] w-20 p-3 -mr-[4px] rounded-tl-xl rounded-bl-xl"
                  }`}
                >
                  <Link
                    to={href}
      ...
      <div>
        <nav
          className={`bg-[#1F1D2B]  lg:hidden fixed w-full bottom-0 left-0 text-3xl text-gray-400 py-7 flex px-10 items-center justify-between rounded-tl-xl rounded-tr-xl`}
        >
          <button>
            <FaUserCog />
          </button>
          ...
          <button
            onClick={toggleMenu}
            className={`${
              showMenu ? "bg-red-300 p-1 text-white rounded-lg" : "p-1"
            }`}
          >
            {/* icono de cierre  */}
